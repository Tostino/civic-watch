...
# ------------------------------------------------------------------- auth
def init(root):
    """Fresh token per process start. Returns the path, never the token."""
...
def state(con, authed):
    """pipeline health without a terminal. Cheap enough to serve on
...
def queues(con):
    ...
    # triage an unnamed voice, highest impact first. The unit is the
    ...
    # (human statements are permanent, and they should look it).
...
def review(con, video_id, name=None, label=None):
    ...
    for r in con.execute(f"""
        ...
        # a fact (applies to admin too - a curation surface may show
...
def correct(con, body, author="admin"):
    """one write, four verbs, over a contiguous idx range."""
...
def decide(con, override_id, decision):
    """accept or reject a queued proposal. Accepting makes it what the
...
def rederive_status(con):
    ...
    # showing a job that will never finish (the stuck thing
...
def _gates(con):
    ...
    unnamed = con.execute(
        ...
        # What is waiting for the DISCOVER steps, which had no measurement at all. For
        # the portal that is meetings already on the county's calendar with no agenda
        # landed yet. Deliberately NOT the 757 past meetings with no agenda: re-running
        # collects none of those, because the county either never posted one or posted
        # an image-only scan. A number a run cannot consume is not a backlog.
    portal = con.execute("""
    ...
    labels = con.execute(
        ...
        "SELECT COUNT(*) FROM speaker_label").fetchone()[0]
    return {
        # Positional: iterating a db.Row yields column NAMES.
...
def redactions(con, status="proposed", limit=50, offset=0, video=None):
    ...
    for r in rows:                                      # positional: a db.Row unpacks to column names
        ...
        # (which this walked into on the first run).
        ...
        title, meeting_id, mdate, mbody, phase = (
            r[11], r[12], r[13], r[14], r[15])
                # Read from text_raw, not the published text: for an applied row `text`
                # is
                # already the marker, and a reviewer would be shown the removal rather
                # than
                # the thing removed. The span's position is sent, not just the span, so
                # the
                # UI marks it in place rather than searching a line that says it twice.
...
def redaction_decide(con, body):
    ...
    if decision == "accept":
                ...
                # touches,
                # so even a single row can outlive a browser's patience: 25 rows across
                # 4
                # recordings committed the status flip, died on the re-index, and left
                # the
                # address gone from the transcript and still in the search index.
