...
CREATE TABLE IF NOT EXISTS people (
    id       integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    -- The ROSTER's key for a board member, and nothing else's. Nullable
    -- because a member of the public does not have one: they are keyed by id
    -- and shown by full_name. It stayed NOT NULL while `people` held 28
    -- commissioners and became wrong the moment everybody named goes in here
    -- - a resident called Sean Poole cannot exist while a commissioner named
    -- Christopher B. Poole owns the surname "Poole".
    --
    -- UNIQUE survives and keeps meaning what it meant, because Postgres lets
    -- any number of rows share a NULL: it protects the roster join and stops
    -- obstructing everybody else. See SPEAKER_PLAN.md section 2.7.
    surname  text,
    full_name text,
    kind     text CHECK (kind IN ('board', 'staff', 'public')),
    UNIQUE (surname)
);
...
CREATE TABLE IF NOT EXISTS organizations (
    id      integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name    text NOT NULL,
    kind    text CHECK (kind IN ('applicant', 'vendor', 'firm', 'agency',
                                 'association', 'subdivision', 'county')),
    UNIQUE (name)
);
